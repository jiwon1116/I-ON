...
<div class="board-wrap">
  ...
  <article class="post">
    <header class="post-head">
      <h1 class="post-title">${free.title}</h1>
        ...
        <div>
          <c:if test="${not empty free.nickname}">
            <span class="js-user" data-nickname="${free.nickname}">
              <a href="${pageContext.request.contextPath}/othermemberprofile/checkprofile?nickname=${free.nickname}">
                ${free.nickname}
              </a>
            </span>
          </c:if>
              ...
              <c:when test="${file.originalFileName.endsWith('.jpg') || file.originalFileName.endsWith('.png') || file.originalFileName.endsWith('.jpeg') || file.originalFileName.endsWith('.gif')}">
                <li>
                  <img class="preview-img"
                       src="${pageContext.request.contextPath}/free/preview?fileName=${file.storedFileName}"
                       alt="${file.originalFileName}" />
                </li>
              </c:when>
              <c:otherwise>
                <li>
                  <a href="${pageContext.request.contextPath}/free/preview?fileName=${file.storedFileName}" target="_blank">
    ...
    <div class="post-actions">
          <div class="stats">
            <span><i class="bi bi-heart-fill text-danger"></i><span id="likeCountDisplay">${free != null ? free.like_count : 0}</span></span>
            <span><i class="bi bi-chat-left-text"></i>${commentList != null ? commentList.size() : 0}</span>
            <span class="text-muted">조회 <span id="viewCount">${free.view_count}</span></span>
          </div>

      <!-- 좋아요 버튼 (기존 스크립트 훅 유지) -->
      <button type="button" class="btn-like ${free != null && free.liked ? 'liked' : ''}" id="likeBtn">
  ...
  <div class="bottom-actions" style="margin-top:16px;">
    <button type="button" class="btn-secondary" onclick="location.href='/free'">목록</button>
  </div>
...
<!-- ===== 기능 스크립트(원본 유지) ===== -->
<script>
  const updateFn = () => { location.href = "/free/update/${free.id}"; }
  const deleteFn = () => {
